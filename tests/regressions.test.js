...
found('Disarmed Breast Fire retains adjacent Damage +2 but cannot inflict Fracture',()=>{
 const c=harness({fed:'secret',zeon:'fed'});c.load('resolveTwinBusterAttack','consumeCriticalOverdrive','criticalEffectsActive','reducedAttackDamage');const a=c.unit('mazinger-z',0,0),d=c.unit('gundam',0,1);c.state.activeUnitId=a.id;a.statuses.disarm=true;c.twinBusterTargets=()=>[d];c.E={...E,rollAttack:(s,a,d,w)=>E.rollAttack(s,a,d,w,()=>.9)};c.resolveTwinBusterAttack(a,a.weapons[1],0);assert.equal(d.hp,d.maxHp-8);assert.equal(d.statuses.fracture,false);
});
