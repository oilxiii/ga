...
test("core movement and rescue costs match the rulebook", () => {
  assert.deepEqual(D.rules.advance, {distance:3,timeline:1});
...
test("each faction receives exactly three unique tactics per phase", () => {
  ...
  s.usedTactics.add(s.hands.fed[0]);
  s.phase=2;
  E.dealTacticHands(s, () => 0.618);
  assert.equal(s.usedTactics.size, 0);
  assert.equal(s.hands.fed.length, 3);
  assert.equal(s.tacticDecks.fed.length, 3);
  assert.ok(s.hands.fed.every(id=>!phaseOne.fed.includes(id)));
  assert.ok(s.hands.zeon.every(id=>!phaseOne.zeon.includes(id)));
});

test("each team refreshes its Tactics independently after all three units pass TL 10", () => {
  const s=E.setupGame(()=>0.2718);
  ...
  assert.equal(s.hands.fed.length,3);
  assert.ok(s.hands.fed.every(id=>!firstFed.includes(id)));
  assert.deepEqual(s.hands.zeon,firstZeon);
  assert.equal(s.tacticDecks.fed.length,3);
  assert.equal(s.tacticDecks.zeon.length,6);
});

test("team Tactic refresh is checked only after that team's activation ends", () => {
  const source=fs.readFileSync(path.join(__dirname,"..","game.js"),"utf8");
  assert.match(source,/refreshTeamTacticsAfterTurn\(unit\.team\);\s*startActivation\(\)/);
  assert.match(source,/state\.tacticCycles\[team\]!==1\|\|!E\.teamPassedTimeline\(state,team,10\)/);
  assert.match(source,/E\.dealTacticHand\(state,team\)/);
  assert.doesNotMatch(source,/state\.currentTick === 11\) \{\s*state\.phase = 2;\s*E\.dealTacticHands/);
});
...
test("battlefield UI uses the compact online title and click-opposite command placement", () => {
  ...
  assert.match(source,/event\.clientX<bounds\.left\+bounds\.width\/2\?"right":"left"/);
...
test("all twelve weapons preserve the Unit Card critical effects", () => {
  const expected={
    ...
    "cracker-grenade":"splashDamage1","bazooka":"damage1",
...
test("Beam Saber Critical grants Gundam Strength against units and Garrisons", () => {
  ...
  assert.match(source,/attacker\.upgrades\.strength\+=1/);
...
test("Cracker Grenade always splashes and Critical raises adjacent damage to two", () => {
  const source=fs.readFileSync(path.join(__dirname,"..","game.js"),"utf8");
...
test("shield absorbs before HP and fracture adds three damage", () => {
  const target = {hp:10,upgrades:{shield:1},statuses:{fracture:true}};
